...
RQDA <- function() {
  ...
  } else
  {
    ".root_rqdagui" <- gwindow(title = gettext("RQDA: Qualitative Data Analysis", domain = "R-RQDA"),parent=c(2,2),
                               width=(gdkScreenWidth()-65),height=(gdkScreenHeight()-65),
...
AddHandler <- function(){
  ...
  addHandlerDoubleclick(.rqda$.CodeofCat,handler=function(h,...) {
    retrieval(Fid=getFileIds(condition=.rqda$TOR,type="coded"),CodeNameWidget=.rqda$.CodeofCat)
  }
  )
  ...
  addHandlerClicked(.rqda$.FileofCat, handler <- function(h, ...) {
    if (length(svalue(.rqda$.FileofCat))>0){
      ...
      names(.rqda$.FileofCat) <- sprintf(gettext("Selected file id is %s", domain = "R-RQDA"),getFileIds("filecat","selected"))
      if (isTRUE(.rqda$SFP)) {
  ...
  addHandlerClicked(.rqda$.FileofCase, handler <- function(h, ...) {
    if (length(svalue(.rqda$.FileofCase))>0) {
      names(.rqda$.FileofCase) <- sprintf(gettext("Selected File id is %s", domain = "R-RQDA"),getFileIds("case","selected"))
    }
    if (isTRUE(.rqda$SFP)) ShowFileProperty(Fid = getFileIds("case", "selected"),focus=FALSE)
  }
  )
